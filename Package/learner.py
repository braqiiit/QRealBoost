...
class weaklearner:
  ...
  def weak_hypothesis(self, X, Dti, no_of_Q):
      ...
      no_of_Q: represents the number of samples that must be chosen from all the M samples
      '''
  ...
  def weak_hypothesis_binary(self, X, y, Dti, no_of_Q):
    '''
    This function is used for returning the partitioning of the X's
    no_of_Q: represents the number of Q samples that must be chosen from all the M samples
    '''
    # we will extract the Q values with top Dti's
    Dti = np.array(Dti)
    ind_max = np.argpartition(Dti, -no_of_Q)[-no_of_Q:]
